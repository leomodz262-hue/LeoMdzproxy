...
def start_mitm():
    ...
    subprocess.run([
        sys.executable, "-c",
        f"import sys; from mitmproxy.tools.main import mitmdump; sys.argv = ['mitmdump', '-s', '{script_path}', '-p', '8089', '--listen-host', '0.0.0.0', '--set', 'block_global=false', '--set', 'ignore_hosts=^(version|freefiremobile-a|cdp|config|rslw0r|firebase).*']; mitmdump()"
    ])
